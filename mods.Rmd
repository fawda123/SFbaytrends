...
renderPlot({
  ...
  p <- ggplot(prdplo(), aes(x = date)) + 
    geom_point(data = rawchl(), aes(y = log10(chl)), size = 0.75) +
...
renderPlot({
  ...
  p <- ggplot() + 
    ...
    geom_line(data = prdplok1(), aes(x = dec_time, group = factor(frms), colour = frms, y = chl), alpha = 0.8) + 
    theme_bw(base_family = 'serif', base_size = 16) + 
    theme(
      legend.position = 'top', 
      legend.title = element_blank(), 
