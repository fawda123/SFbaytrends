...
* `gam6`: chl ~ year + s(doy) + s(year, k = *large*)
...
__Select the station with the drop down menu for the summary of all four GAM chlorophyll models.__  
...
The two plots show the predicted trends from each of the four models.  The first plot shows estimed chlorophyll by day of year with separate lines for each year.  The second plot show predictions for the four models across the entire time series. 

```{r}
mods <- reactive({
  modssta_chl %>% filter(station %in% input$station)
})
...
```{r, echo = F}
selectInput('mod', "Select a model:", unique(modssta_chl$modi))
```
```{r}
# smoother stats of GAMs
renderTable({
...
column(4, selectInput('gamtyp', 'Select gam type:', 
                      choices = list(
                        gam1 = "log10(value) ~ dec_time + s(dec_time) + s(doy, bs = 'cc')", 
                        gam2 = "log10(value) ~ dec_time + s(dec_time) + s(doy, bs = 'cc') + ti(dec_time, doy, bs = c('tp', 'cc'))" 
                          )
                      )
       )
...
modsk1 <- reactive({
  ...
  modsk1 <- frms %>% 
    ...
    group_by(frms) %>% 
    mutate(
      data = purrr::map(., function(x) mods$data[[1]]),
...
renderPlot({
  ...
  req(prdplok1())
  rawval <- modsk1()$data[[1]]

  p <- ggplot() + 
    geom_point(data = rawval, aes(x = dec_time, y = log10(value)), size = 0.5) +
    geom_line(data = prdplok1(), aes(x = dec_time, group = factor(frms), colour = frms, y = value), alpha = 0.8, size = 0.75) + 
    theme_bw(base_family = 'serif', base_size = 16) + 
    theme(
      legend.position = 'top', 
      legend.title = element_blank(), 
