...
mapselplo <- reactive({
  ...
  if(parameter %in% c('chl'))
    p2 <- p2 + 
      scale_y_log10(ylb)
  if(parameter %in% c('docalc', 'gpp'))
    p2 <- p2 + 
...
Generalized additive models (GAMs) were developed to describe trends in `r renderText(txtlb())` at each of the monitoring stations in South Bay.  The station can be chosen from the drop down menus.  One GAM was developed for the time series at each station to model `r renderText(txtlb())` as a function of time, where time is measured as a continuous year value.

* `S`: `r renderText(input$parameter)` ~  s(cont_year, k = *large*)

The `cont_year` variable is measured as a continuous numeric value for the annual effect (e.g., January 1st, 2000 is 2000.0, July 1st, 2000 is 2000.5, etc.). The function `s()` models `year` as a smoothed, non-linear variable.  The `k` value is a theoretical upper limit on the number knots used to fit the smoother for `cont_year`.  The optimal number of knots are determined automatically using generalized cross-validation as implemented with the `mgcv()` function.  Using a sufficiently large number for `k` ensures the model is not underfit to the data. The upper limit of `k` was chosen as 12 times the number of years of data at each station (~360).
...
plotOutput('prdseries', height = "370px")
```
