...
mapselplo <- reactive({
  ...
  p2 <- rawplo %>% 
    ...
    .[[1]] %>% 
    .[[1]] + 
    scale_y_log10('Chlorophyll-a (ug/L)') + 
...
prddoy <- reactive({
  ...
  out <- show_prddoy(mods = mods, ylab = 'Chlorophyll-a (ug/L)')
...
prdseries <- reactive({
  ...
  out <- show_prdseries(mods = mods, ylab = 'Chlorophyll-a (ug/L)')
...
prdseason <- reactive({
  ...
  out <- show_prdseason(mods = mods, ylab = 'Chlorophyll-a (ug/L)')
...
prd3d <- reactive({
  ...
  out <- show_prd3d(mods = modsind, ylab = 'Chlorophyll-a (ug/L)')
...
perchg <- reactive({
  ...
  out <- show_perchg(mods = modsind, baseyr = yrbs, testyr = yrts, ylab = 'Chlorophyll-a (ug/L)')
...
avgseason <- reactive({
  ...
  out <- show_avgseason(mods = modsind, doystr = dytr[1] , doyend = dytr[2], yrstr = yrtr[1], yrend = yrtr[2], 
...
## GAMs of log-chlorophyll with annual, seasonal trends {.tabset}
...
column(12,
  ...
  column(6,
    sliderInput('yrtr', 'Select years for across-year trend estimate:', min = min(yrs), max = max(yrs), value = c(2010, 2017), sep = "", step = 1)
  )
)
```
