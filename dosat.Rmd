...
toplo <- reactive({
  ...
  sel <- sel + scale_y_continuous('DO (% sat)')
...
## GAMs of dissolved oxygen (% sat) with annual, seasonal trends
...
```{r}
mods <- reactive({
  modssta_dosat %>% filter(station %in% input$station)
})
...
```{r, echo = F}
selectInput('mod', "Select a model:", unique(modssta_dosat$modi))
```
```{r}
# smoother stats of GAMs
renderTable({
