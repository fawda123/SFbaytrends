...
## GAMs of gross primary production (GPP, mg C m-2 d-1) with annual, seasonal trends
...
renderPlot({
  p <- ggplot(prdplo(), aes(x = doy, group = factor(yr), colour = yr)) + 
    ...
    scale_y_continuous('GPP (mg C m-2 d-1)') +
...
renderPlotly({
  ...
  scene <- list(
    ...
    zaxis = list(title = 'GPP (mg C m-2 d-1)'),
    roughness = .01,
    ambient = 0.9
  )
