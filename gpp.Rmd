...
toplo <- reactive({
  ...
  if(input$log){
    ...
    sel <- sel + scale_y_log10('GPP (mg C m-2 d-1)')
    
  } else {
    
    sel <- sel + scale_y_continuous('GPP (mg C m-2 d-1)')
...
## GAMs of log-transformed gross primary production (GPP, mg C m-2 d-1) with annual, seasonal trends

*Note:* All plots below are shown with model results in log-space, although plot axis-scaling is arithmetic. 

Generalized additive models (GAMs) were developed to describe trends in GPP at each of the monitoring stations in South Bay.  The station and selected model can be chosen from the drop down menus.  Four types of GAMs were developed for the time series at each station to model GPP as a function of time, where time is measured as an annual and seasonal effect.  The four models descibe the time components differently and represent increasing levels of complexity to describe the GPP trend:

* `gam0`: GPP ~ year + s(doy)

* `gam1`: GPP ~ year + s(doy) + s(year)

* `gam2`: GPP ~ year + s(doy) + s(year) + ti(doy, year)

* `gam6`: GPP ~ year + s(doy) + s(year, k = *large*)
...
__Select the station with the drop down menu for the summary of all four GAM GPP models.__  
...
The two plots show the predicted trends from each of the four models.  The first plot shows estimed GPP by day of year with separate lines for each year.  The second plot show predictions for the four models across the entire time series. 

```{r}
mods <- reactive({
  modssta_gpp %>% filter(station %in% input$station)
})
...
renderPlot({
  p <- ggplot(prdplo(), aes(x = doy, group = factor(yr), colour = yr)) + 
    ...
    scale_y_continuous('log10(GPP) (mg C m-2 d-1)') +
...
renderPlot({
  ...
  p <- ggplot(prdplo(), aes(x = date)) + 
    geom_point(data = rawval(), aes(y = log10(value)), size = 0.5) +
...
renderPlot({
  ...
  p <- ggplot() + 
    ...
    geom_point(data = rawval, aes(x = date, y = log10(value)), size = 0.5) +
...
column(12, 
  column(6,
    sliderInput('dytr', 'Select day-of-year range to define seasons:', min = 1, max = 365, value = c(21, 355))
  ),
  column(6,
    renderUI({
      ...
      sliderInput('yrtr', 'Select years for across-year trend estimate:', min = min(yrs), max = max(yrs), value = c(2010, 2017), sep = "")
...
renderPlotly({
  ...
  scene <- list(
    ...
    zaxis = list(title = 'log10(GPP) (mg C m-2 d-1)'),
    roughness = .01,
    ambient = 0.9
  )
  
  p <- plot_ly(z = ~logvalue, height = 600) %>% 
